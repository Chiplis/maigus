//! Maigus - Magic: The Gathering Rules Engine
//!
...
use maigus::cards::builders::CardTextError;
use maigus::decision::{CliDecisionMaker, DecisionRouter, init_input_manager, read_input};
...
fn parse_args() -> GameArgs {
    ...
    while i < args.len() {
        match args[i].as_str() {
            ...
            "--help" | "-h" => {
                println!("Maigus - MTG Rules Engine");
                println!();
                println!("Usage: maigus [OPTIONS]");
                println!();
                println!("Options:");
                println!(
