...
fn parse_args(root_dir: &Path) -> Result<Args, String> {
    let mut threshold = env::var("MAIGUS_PARSER_SEMANTIC_THRESHOLD")
        .or_else(|_| env::var("MAIGUS_WASM_SEMANTIC_THRESHOLD"))
        .ok();
    let mut dims = env::var("MAIGUS_WASM_SEMANTIC_DIMS")
        .ok()
        .and_then(|raw| raw.parse::<usize>().ok())
        .unwrap_or(384usize);
    let mut min_cluster_size = env::var("MAIGUS_WASM_MIN_CLUSTER_SIZE")
        .ok()
        .and_then(|raw| raw.parse::<usize>().ok())
        .unwrap_or(1usize);
    let mut limit = None;
    ...
    let threshold = threshold.ok_or_else(|| {
        "missing threshold: pass --threshold or set MAIGUS_PARSER_SEMANTIC_THRESHOLD".to_string()
    })?;
    Ok(Args {
        threshold,
        dims,
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mismatch_names_file = tmp_dir.join(format!("maigus_wasm_mismatch_names_{run_id}.txt"));
    let filtered_skip_names_file = reports_dir.join(format!("maigus_wasm_skip_names_{run_id}.txt"));
    let failures_report = tmp_dir.join(format!("maigus_wasm_threshold_failures_{run_id}.json"));
    let cluster_report = tmp_dir.join(format!("maigus_wasm_cluster_report_{run_id}.json"));
    let audits_report = tmp_dir.join(format!("maigus_wasm_card_audits_{run_id}.json"));
    let mismatch_report = reports_dir.join(format!(
        "maigus_wasm_semantic_mismatch_report_{run_id}.json"
    ));
    let unparsable_report =
        reports_dir.join(format!("maigus_wasm_unparsable_clusters_{run_id}.json"));
