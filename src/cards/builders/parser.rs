...
fn parser_semantic_guard_enabled() -> bool {
    std::env::var("MAIGUS_PARSER_SEMANTIC_GUARD")
        .map(|value| {
            !matches!(
                value.as_str(),
        ...
        .unwrap_or_else(|_| {
            option_env!("MAIGUS_PARSER_SEMANTIC_GUARD_DEFAULT")
                .map(|value| {
                    !matches!(
                        value,
                        "0" | "false" | "FALSE" | "no" | "NO" | "off" | "OFF"
...
fn parser_semantic_embedding_dims() -> usize {
    std::env::var("MAIGUS_PARSER_SEMANTIC_DIMS")
        .ok()
        .and_then(|raw| raw.parse::<usize>().ok())
        .filter(|dims| *dims > 0)
        .or_else(|| {
            option_env!("MAIGUS_PARSER_SEMANTIC_DIMS_DEFAULT")
                .and_then(|raw| raw.parse::<usize>().ok())
                .filter(|dims| *dims > 0)
        })
        .unwrap_or(384)
...
fn parser_semantic_embedding_threshold() -> f32 {
    std::env::var("MAIGUS_PARSER_SEMANTIC_THRESHOLD")
        .ok()
        .and_then(|raw| raw.parse::<f32>().ok())
        .or_else(|| {
            option_env!("MAIGUS_PARSER_SEMANTIC_THRESHOLD_DEFAULT")
                .and_then(|raw| raw.parse::<f32>().ok())
        })
        .unwrap_or(0.9)
}
