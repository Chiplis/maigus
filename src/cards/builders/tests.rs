...
fn parse_semantic_guard_rejects_round_trip_drift_when_enabled() {
    let prev_guard = std::env::var("MAIGUS_PARSER_SEMANTIC_GUARD").ok();
    let prev_dims = std::env::var("MAIGUS_PARSER_SEMANTIC_DIMS").ok();
    let prev_threshold = std::env::var("MAIGUS_PARSER_SEMANTIC_THRESHOLD").ok();
    unsafe {
        std::env::set_var("MAIGUS_PARSER_SEMANTIC_GUARD", "1");
        std::env::set_var("MAIGUS_PARSER_SEMANTIC_DIMS", "384");
        std::env::set_var("MAIGUS_PARSER_SEMANTIC_THRESHOLD", "0.9");
    }
    ...
    match prev_guard {
        Some(value) => unsafe {
            std::env::set_var("MAIGUS_PARSER_SEMANTIC_GUARD", value);
        },
        None => unsafe {
            std::env::remove_var("MAIGUS_PARSER_SEMANTIC_GUARD");
        },
    }
    match prev_dims {
        Some(value) => unsafe {
            std::env::set_var("MAIGUS_PARSER_SEMANTIC_DIMS", value);
        },
        None => unsafe {
            std::env::remove_var("MAIGUS_PARSER_SEMANTIC_DIMS");
        },
    }
    match prev_threshold {
        Some(value) => unsafe {
            std::env::set_var("MAIGUS_PARSER_SEMANTIC_THRESHOLD", value);
        },
        None => unsafe {
            std::env::remove_var("MAIGUS_PARSER_SEMANTIC_THRESHOLD");
        },
    }
