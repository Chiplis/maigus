    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent side="right" className="w-[400px] bg-[#0b1017] border-game-line">
        <SheetHeader>
          ...
          <span className="text-muted-foreground text-[11px]">Latest 120 entries</span>
        </SheetHeader>
        <ScrollArea className="h-[calc(100vh-100px)] mt-4">
          <ul className="list-none p-0 m-0 flex flex-col gap-0.5">
            {logEntries.map((entry, i) => (
              <li
                key={i}
                className={`text-[11px] leading-tight py-0.5 px-1 border-b border-game-line-2 ${
            ...
            {logEntries.length === 0 && (
              <li className="text-muted-foreground text-[12px] italic p-3 text-center">
                No log entries yet
              </li>
            )}
          </ul>
