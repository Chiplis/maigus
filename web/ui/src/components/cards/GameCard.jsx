  ...
  return (
    <div
      className={cn(
        ...
        compact && "w-[96px] min-w-[96px] min-h-[134px] p-1 text-[11px]",
        !compact && "w-[124px] min-w-[124px] min-h-[172px]",
        card.tapped && "tapped",
        isPlayable && "playable",
    ...
    >
      ...
      <span className="absolute top-0 left-0 right-0 mt-0 bg-[rgba(16,24,35,0.85)] px-1.5 py-0.5 z-2 whitespace-nowrap overflow-hidden text-ellipsis text-shadow-[0_1px_1px_rgba(0,0,0,0.85)]">
        {groupSize > 1 && (
          <span className="inline mr-1 border border-[rgba(245,198,112,0.9)] bg-[rgba(16,24,35,0.92)] text-[#f5d08b] text-[10px] font-bold leading-none px-1 py-0.5 rounded-sm tracking-wide">
            x{groupSize}
          </span>
        )}
        {name}
      ...
      {card.power_toughness && (
        <span className="absolute bottom-1 right-1 border border-[rgba(245,198,112,0.9)] bg-[rgba(16,24,35,0.92)] text-[#f5d08b] text-[10px] font-bold leading-none px-1 py-0.5 rounded-sm z-2 tracking-wide">
          {card.power_toughness}
        </span>
      )}
      ...
      {scryfallUrl && (
        <a
          className="absolute top-1 right-1 border border-[#4e6f93] bg-[#0a1118] text-[#9ec3ea] no-underline uppercase text-[9px] tracking-wide px-1 py-px rounded-sm leading-tight z-2 opacity-0 hover:opacity-100 group-hover:opacity-100 transition-opacity"
          href={scryfallUrl}
          target="_blank"
